...
| Game | What it is |
|------|------------|
| **Emberfall** | Full **2D pixel soulsbourne** on a large, procedurally-generated **island** (irregular coastline, beaches, forests, mountains, lakes) with Terraria-style art & lighting. Choose a **race** (Human/Orc/Elf/Dwarf/Undead — each with buffs & weaknesses) **and** a **role** (Knight/Wizard/Archer/Rogue/Cleric — each with its own weapon & skill). **7 stats**, **mana & spells**, **light + heavy attacks**, **parry** (right-click/F) & **dodge-roll**, a **hotbar**, gear you loot & equip, bonfires to rest/level, an **island map** you open to **fast-travel**, and **8 bosses each with a distinct design & moveset** (knight, dragon, golem, wraith, hydra, demon…). |
| **Battle Drop** | 2D battle royale on a large **island** (not a square) with named **POIs** (Lighthouse, Fishing Village, Old Ruins, Hilltop Fort…). Everyone drops in with **nothing but bare fists** and must loot guns & shields. **Hotbar** of up to 3 weapons (1/2/3 or Q, reload R). Human-like bots loot up, take cover, react with delay, miss, keep their distance, and flee when hurt. Full island **map** (M). Survive the shrinking storm. |
